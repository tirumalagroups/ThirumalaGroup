...
// Replacements in LoanEntry.tsx based on visual hierarchy:
// H1 -> peek-h1
loanEntry = loanEntry.replace(/finance-h1/g, 'peek-h1');

// Form Section Headers (were finance-header-time uppercase) -> peek-h3 uppercase
loanEntry = loanEntry.replace(/finance-header-time uppercase/g, 'peek-h3 uppercase');

// Small Labels under headers (were finance-small-label) -> peek-small-10
loanEntry = loanEntry.replace(/finance-small-label/g, 'peek-small-10');

// Buttons (were finance-button) -> peek-button
loanEntry = loanEntry.replace(/finance-button/g, 'peek-button');

// CLEAR button (was finance-header-time without uppercase, but it has X icon)
// Actually let's just make all buttons use peek-button uppercase if they are actions.
// "CLEAR" button className="... finance-header-time"
loanEntry = loanEntry.replace(/finance-header-time(.*?>\s*<X.*?CLEAR)/gs, 'peek-button uppercase$1');

// Input Labels (were finance-caption uppercase) -> peek-label uppercase
loanEntry = loanEntry.replace(/finance-caption uppercase/g, 'peek-label uppercase');

// Input values / select text (were finance-header-time or finance-input) -> let's map them to peek-caption-12 or just tailwind classes
loanEntry = loanEntry.replace(/finance-header-time/g, 'peek-caption-12'); 
loanEntry = loanEntry.replace(/finance-input/g, 'peek-button'); // inputs use font-medium 14px
